...
The minifridge placements are derived from `profile-garderobe`, not stored as unrelated absolute coordinates. Placement A continues the free end cap and points the marked door edge away from the profile. Placement B keeps the refrigerator back coplanar with the kitchen-side profile wall; a 15 cm offset beyond the profile end is required so its full 50 cm operating zone does not enter the estimated fixed kitchen return. Both the 40 × 43 cm footprint and the operating zone must remain strictly inside the apartment and clear of fixed kitchen fixtures, doors, bathroom, interior walls, and every loose-furniture footprint. Because the source plan is not to scale, these clearances must be checked on site before final installation.
