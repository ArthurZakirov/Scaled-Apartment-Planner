...
The PAX opening has a 45 cm geometric access strip. A scenario is invalid if any other furniture footprint intersects that strip, even when the furniture footprints themselves do not overlap. This catches the previous common-sense failure where a bed only 4.1 cm from the open PAX was labeled geometrically valid. Depending on the orientation, the cabinet moves to the free bed end or another corner-side position; arrangements where the bed blocks access remain diagnostic-only.
