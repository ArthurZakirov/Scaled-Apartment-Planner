...
test('browser resolves all 36 product-variant scenarios', () => {
  const furniture = resolveScenarioData(scenarios, catalog);
  assert.equal(furniture.layouts.length, 36);
  assert.equal(new Set(furniture.layouts.map((layout) => layout.id)).size, 36);
});
